...
export class OpenaiService {
    ...
    private readonly embedRateLimiter: RateLimiterRedis;
    private readonly defaultKeys = [];


    constructor(private appConfig: AppConfigService, @Inject(REDIS) private redis: Redis) {
        this.defaultKeys = [
        ...
        // Rate limiter for 1000000 tokens / min
        this.embedRateLimiter = new RateLimiterRedis({
    ...
    async getEmbedding(
        ...
        // Rate limiter check
        try {
            let tokens = this.getTokenCount(input) + 1;
            await this.embedRateLimiter.consume(`openai-emd-${openAiKeyHash}`, tokens);
        } catch (err) {
