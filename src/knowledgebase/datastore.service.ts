...
export class DataStoreService {
   ...
   * @returns
   */
  async insertToDataStoreAndCreateEmbeddings(
    data: KbDataStore,
    skipEmbeding = false,
  ) {
    let dsItem;
  ...
  async addCustomDataToKnowledgebase(
    ...
    let dsItem: KbDataStore = {
      ...
      title: data.q,
      content: data.a,
      type: data.type || DataStoreType.CUSTOM,
      status: DataStoreStatus.CREATED,
    ...
    dsItem = await this.insertToDataStoreAndCreateEmbeddings(
      dsItem,
      data.skipEmbedding || false,
    );
    return dsItem;
  }
