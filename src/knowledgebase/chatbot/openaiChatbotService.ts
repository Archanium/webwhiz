...
import {CELERY_CLIENT, CeleryClientQueue, CeleryClientService,} from '../../common/celery/celery-client.module';
import {retryWithBackoff} from '../../common/utils';
import {ChatGptPromptMessages, ChatGTPResponse, OpenaiService,} from '../../openai/openai.service';
import {KnowledgebaseDbService} from '../knowledgebase-db.service';
import {ChatQueryAnswer, Chunk, CHUNK_SIZE, ChunkStatus, CustomKeyData,} from '../knowledgebase.schema';
...
export class OpenaiChatbotService {
    ...
    async getEmbeddingsForChunk(chunk: Chunk, customKeys?: CustomKeyData) {
        ...
        return await retryWithBackoff(
            async () => {
                return await this.openaiService.getEmbedding(
                    text,
                    this.getCustomKeys(customKeys),
                );
            },
            undefined,
            10, // max 10 retries (2 ** 10 * 500)
            500
        );
    }
    ...
    getChatGptPrompt(
        ...
        const remainingTokens = maxTokenLimit - emptyPromptTokens - 500; // 500 for response
