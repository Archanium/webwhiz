...
export class ChatbotService {
  ...
  private async getChatSessionDataFromCache(sessionId: string) {
    ...
    const data = await this.redis.get(sessionKey).catch((error: any) => {
      console.log(error);
      return null;
    });
    if (!data) {
      console.log({ message: 'session not found from redis', sessionId });
      const session = await this.kbDbService.getChatSessionById(sId);
      if (!session) {
        console.log({ message: 'session not found from database', sessionId });
